...
def main(argv):
    ...
    for i in range(homeCount):
        ...
        home_name = ''.join(choice(characters) for i in range(6))
    ...
    for i in range(userCount):
        ...
        z = randint(0,len(homeIDs)) if assigned else -1
        assignedHome = homeIDs[z] if assigned else "unassigned"
    ...
    for i in range(taskCount):
        ...
        params = urllib.parse.urlencode({'name': choice(taskNames), 'deadline': deadline, 'assigneeName': assignedUserName, 'assignee': assignedUserID, 'completed': str(completed).lower(), 'notes': notes, 'rotate': choice(rotate)})
